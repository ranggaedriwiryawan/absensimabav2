...
(function () {
  ...
  // robust TL parser (Tempat, 21 Agustus 2005  OR  TEMPAT 21 AGUSTUS 2005)
  function parseTL(raw){
    ...
    const s=String(raw).trim(); let tempat='-', tanggalLabel=s;
    if(s.includes(',')){ const [plc,rest]=s.split(',',2); tempat=(plc||'').trim()||'-'; tanggalLabel=(rest||'').trim()||'-'; }
    else { const t=s.split(/\s+/); if(t.length>=4){ const d=t[t.length-3], m=t[t.length-2], y=t[t.length-1]; tempat=t.slice(0,t.length-3).join(' ')||'-'; tanggalLabel=`${d} ${m} ${y}`; } }
    ...
    let usia='-'; try{ const p=tanggalLabel.split(/\s+/); if(p.length>=3){ const d=+p[0]; const b=bulan[(p[1]||'').toLowerCase()]||null; const y=+p[2];
      if(d && b && y){ const dt=new Intl.DateTimeFormat('id-ID',{timeZone:'Asia/Jakarta',year:'numeric',month:'2-digit',day:'2-digit'}).formatToParts(new Date());
        const Y=+dt.find(x=>x.type==='year').value, M=+dt.find(x=>x.type==='month').value, D=+dt.find(x=>x.type==='day').value;
        usia = Y - y - ((M<b || (M===b && D<d)) ? 1 : 0); if(usia<0||usia>120) usia='-'; } } }catch(_){}
    return {tempat, tanggalLabel, usia};
  }

  // payload "Nama; Prodi; Alamat; Tempat+Tgl; Hobi; Motto; Riwayat"
  function parsePayloadSemicolon(text){
    if(!text || !text.includes(';')) return null;
    const p=text.split(';').map(s=>s.trim());
    if(p.length<2) return null;
    const nama=p[0]??'', prodi=p[1]??'', alamat=p[2]??'', tglRaw=p[3]??'', hobi=p[4]??'', motto=p[5]??'', riwayat=(p[6]??'').toString();
    const tl=parseTL(tglRaw);
    return {
      qr:text,
      nama:nama||'-', prodi:prodi||'-', alamat:alamat||'-',
      tglLahirRaw:tglRaw||'-', tempatLahir:tl.tempat||'-', tglLahirTanggal:tl.tanggalLabel||'-', usia:(tl.usia??'-')===''?'-':tl.usia,
      hobi:hobi||'-', motto:motto||'-', riwayat:riwayat===''?'-':riwayat
    };
  }

  // lookup ke master: QR CODE -> fallback Nama;Prodi
  async function findMabaByAny(qrText){
    ...
    const nT = normUpper(namaPart);
    const pT = normUpper(prodiPart || '');

    const all=await DB.listMaster();
    return all.find(r=>{
      const n=normUpper(r.nama), p=normUpper(r.prodi);
      return pT ? (n===nT && p===pT) : (n===nT);
    }) || null;
  }
  ...
  async function processDecoded(qrText, ui){
    ...
    // 2) Jika bukan payload lengkap → cari di master (QR CODE / Nama;Prodi)
    if(!maba || (maba.nama==='-' && maba.prodi==='-')){
      const found = await findMabaByAny(scanned);
      if(!found){
        const tanggal = Utils.todayWIB(), timeStr = Utils.timeWIB();
    ...
    const {slot, inWindow} = Utils.detectSlot(window.APP_CONFIG.timeWindowsWIB);
    const tanggal = Utils.todayWIB(), timeStr = Utils.timeWIB(), iso = Utils.isoWIB();
    const user = JSON.parse(sessionStorage.getItem('user') || '{}');

    const uniqueKey = `${maba.qr}|${tanggal}|${slot}`;
    const existed = await DB.findLogByUnique(uniqueKey);
    if (existed) { renderCard(ui, maba, existed, 'warn'); return; }
  ...
  window.initQrScanner = function (opts) {
    ...
    if (!window.Html5QrcodeScanner || !window.Html5QrcodeScanType) {
      ui.status && (ui.status.textContent = 'Gagal memuat library scanner.');
      return;
    }
    ...
    scanner.render(
      (decodedText) => { ui.status && (ui.status.textContent = `QR: ${decodedText}`); processDecoded(decodedText, ui); },
      (_err) => {}
    );
  };
})();
